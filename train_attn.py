...
#add folder path to the system path
...
model_savefile = "Projects/ViZDoom/BLG521E/" + config + "/" + config + "_baseline_baseModel.pth"
save_model = True
load_model = False
skip_learning = False
...
if not os.path.exists("Projects/ViZDoom/BLG521E/" + config):
    os.makedirs("Projects/ViZDoom/BLG521E/" + config)
...
def run(game, agent, actions, num_epochs, frame_repeat, steps_per_epoch=2000):
    ...
    for epoch in range(num_epochs):
        ...
        video = cv2.VideoWriter("Projects/ViZDoom/BLG521E/" + config + "/" + config + "_Epoch_" + str(epoch) + ".avi",   
                         cv2.VideoWriter_fourcc(*'MJPG'), 
                         8, (640,480))
        video_frames = []

        for _ in range(steps_per_epoch):
            state_original = game.get_state().screen_buffer
...
class DQNAgent:
    ...
    def get_action(self, state, mod = "train"):
        if (np.random.uniform() < self.epsilon) & (mod=="train"):
            ...
            if(mod=="train"):
                ...
                logits, attn = self.q_net(state, mod)
...
if __name__ == "__main__":
    ...
    # Reinitialize the agent with epsilon set to 0
    ...
    for ep in range(episodes_to_watch):
        ...
        if(skip_learning): 
            path = "Projects/ViZDoom/BLG521E/" + config + "/" + config + "_Final_E"+str(ep)+".avi"
            path_attn = "Projects/ViZDoom/BLG521E/" + config + "/" + config + "Attn_Final_E" + str(ep) + ".avi"
        else: 
            path = "Projects/ViZDoom/BLG521E/" + config + "/" + config + "_Final_E"+str(ep)+".avi"
            path_attn = "Projects/ViZDoom/BLG521E/" + config + "/" + config + "Attn_Final_E"+str(ep)+".avi"
        ...
        while not game.is_episode_finished():
            ...
            best_action_index, attn = agent.get_action(state, "test")
